...
import { ProfileApiService } from '../../../superadmin/profile/services/profile-api.service'; // Pastikan path ini benar!
import Swal from 'sweetalert2';

@Component({
  selector: 'app-hrd-profile',
  standalone: false,
  templateUrl: './hrd-profile.component.html',
  styleUrls: ['./hrd-profile.component.scss']
})
export class HrdProfileComponent implements OnInit {
    ...
    this.profileApi.getProfile().subscribe({
      next: (res: any) => {
        if (res.status === 'success') {
          ...
          this.profileData.role = res.data.role?.name || res.data.role || 'Admin HRD';
        }
      ...
      error: (err: any) => {
        ...
        if (localUserStr) {
          try {
            ...
            this.profileData.role = parsed.role?.name || parsed.role || 'Admin HRD';
          } catch (e) {
  ...
  updateProfile() {
    ...
    this.profileApi.updateProfile({ name: this.profileData.name }).subscribe({
      next: (res: any) => {
        ...
        // 1. CARI KEY LOCAL STORAGE YANG DIPAKAI APLIKASI
        ...
        if (localUserStr) {
          ...
          // 3. SIMPAN KEMBALI KE LOCAL STORAGE
          localStorage.setItem(userKey, JSON.stringify(localUser));

          // 4. 👇 SANGAT PENTING: TEMBAKKAN SINYAL KE SIDEBAR!
