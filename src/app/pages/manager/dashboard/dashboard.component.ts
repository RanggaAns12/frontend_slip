...
export class DashboardComponent implements OnInit, AfterViewInit {
  ...
  // Variabel Data Murni (Dimulai dari 0)
  totalEmployees = 0;
  activeEmployees = 0;
  inactiveEmployees = 0;
  totalUsers = 0;
  ...
  get activeEmployeesPercentage(): string {
    if (this.totalEmployees === 0) return '0.0';
    return ((this.activeEmployees / this.totalEmployees) * 100).toFixed(1);
  }
  ...
  fetchDashboardData() {
    ...
    this.http.get<any>(apiUrl).subscribe({
      next: (res) => {
        ...
        // Menangkap data baru dari backend (Jika backend belum mengirim field ini, maka akan 0)
        ...
        setTimeout(() => {
          ...
          // Doughnut Chart sekarang menampilkan Rasio [Aktif, Non-Aktif]
  ...
  // 🔴 RENDER BAR CHART (Statistik Kehadiran Tetap Dipertahankan)
  ...
  // 🔴 RENDER DOUGHNUT CHART (Sekarang jadi Rasio Status Karyawan)
  renderDoughnutChart(statusArray: any[]) {
    ...
    this.doughnutChart = new Chart(this.statusChartRef.nativeElement, {
      ...
      data: {
        ...
        datasets: [{
          data: statusArray,
          backgroundColor: ['#10b981', '#f43f5e'], // Emerald untuk Aktif, Rose untuk Non-Aktif
          borderWidth: 0,
          hoverOffset: 4
        }]
      },
